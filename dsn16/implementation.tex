...
\myparagraph{Shared memory commit} To implement the RC memory model, we use shared memory commit of  Dthreads~\cite{dthreads-sosp-2011} (originally proposed in distributed shared memory architectures such as TreadMarks~\cite{treadmark} and Munin~\cite{munin}) that allows threads to communicate at well-defined synchronization points.
...
\myparagraph{Input} In addition to providing wrappers for {\tt pthreads} and {\tt malloc} related API calls, we also implemented shim layer for a number of {\tt glibc} library calls for reading the input. In particular, we provide wrappers for {\tt mmap} \& {\tt fread} to record the data-flow from the input. To do so, the threading library differentiates between the {\tt mmap} calls made by the library itself and the target application. This allows us to record the mapping of the input file in the input address space. 


\subsection{OS Support for \intelpt}

To obtain the control flow dependencies, we use the new Intel ISA Extension, called Processor Trace (PT), recently released as part of the Broadwell (also in Skylake) microarchitecture. In Linux this processor feature is exposed to the user-space as a Performance Measuring Unit (PMU) in the {\tt perf} event interface. In addition to the threading library, \projecttitle provides OS support for \intelpt using Intel PMU to derive the control flow dependencies. Figure~\ref{fig:pt-OS-support} shows the architecture for the OS support for \intelpt.
 


In particular, the Perf interface on Linux consists of a syscall, which gives back a file
...
We create such a croup exclusively for the application under test. This is done because
