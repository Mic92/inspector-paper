...
\myparagraph{Vertices: sub-computations (sub-computations)} We define a {\em sub-computation}  as the sequence of instructions
executed by a thread between two \pthreads synchronization API calls. We model an execution of thread $t$ as a sequence of sub-computations
($L_t$). 


\myparagraph{Edges: dependencies} We distinguish between three kinds of edges: control, synchronization, and data-dependence edges. We next described these edges.
...
\myparagraph{Control edges} {\em Control edges}  are used to record the intra-thread causal order between sub-computations of the same thread
based on their execution order, and also control flow within each sub-computation at the granularity of code {\em thunks}. We derive control edges to record the intra-thread causal order between sub-computations of the same thread based on their execution order. Sub-computations in a thread are totally ordered based on their execution order
...
Furthermore, we also record all control path taken by the thread within each sub-computation during the execution, which we refer to as code {\em thunks}.  
We denote a thunk $L_t[\alpha].T_h[i]$ to a sequence of instructions between the branches within each subcomputation $L_t[\alpha]$. 


\myparagraph{Synchronization edges}  {\em Synchronization edges}   are
...
the thread while executing the sub-computation. {\em Data dependence edges} are used to establish the {\em update-use relationship} between sub-computations. The update-use relationship exists between two sub-computations if they can be  ordered based on the happens-before relationship, and the write-set of the  precedent  sub-computations intersects with the read-set of the antecedent sub-computations.
